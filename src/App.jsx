...
// ─── PANEL DE REVISIÓN (compartido desktop + móvil) ──────────────────────────
function RevisionPanel({ preguntas, respuestas }) {
  return (
...
function Resultado() {
  ...
  const [vistaMovil, setVistaMovil] = useState("score"); // "score" | "revision"
  ...
  return (
    ...
    >
      ...
      {vistaMovil === "revision" && (
        ...
        >
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-b border-slate-800 flex-shrink-0">
            <button
              onClick={() => setVistaMovil("score")}
              className="w-9 h-9 rounded-xl border border-slate-700 flex items-center justify-center text-slate-400 hover:text-slate-200 hover:border-slate-500 transition-all"
            >
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
            ...
            <div>
              <p className="text-white font-bold text-sm">Revisión de respuestas</p>
              <p className="text-slate-500 text-xs">{correctas} correctas · {total - correctas} incorrectas</p>
            </div>
          </div>
          {/* Contenido revisión */}
          <div className="flex-1 overflow-y-auto px-4 py-6">
            <RevisionPanel preguntas={preguntas} respuestas={respuestas} />
          </div>
        </motion.div>
      )}

      {/* ── MÓVIL: pantalla de score ── */}
      {vistaMovil === "score" && (
        <motion.div
          initial={{ opacity: 0, x: -40 }} animate={{ opacity: 1, x: 0 }}
          className="md:hidden flex flex-col items-center justify-start w-full h-full overflow-y-auto px-6 py-10"
        >
          <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 180, delay: 0.1 }} className="text-6xl mb-4">
            {aprobado ? "🎉" : "📚"}
          </motion.div>
          <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
            className={`text-3xl font-black mb-2 ${aprobado ? "text-emerald-400" : "text-red-400"}`}>
            {aprobado ? "¡Aprobaste!" : "No aprobaste"}
          </motion.h2>
          <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
            className="text-slate-500 text-center text-sm mb-6 leading-relaxed max-w-xs">
          ...
          <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }}
            className={`w-36 h-36 rounded-full border-4 flex flex-col items-center justify-center mb-6 ${
          ...
          <div className="flex gap-8 mb-8">
            <div className="text-center">
              <div className="text-3xl font-black text-emerald-400">{correctas}</div>
              <div className="text-xs text-slate-500 mt-1">Correctas</div>
            </div>
            <div className="text-center">
              <div className="text-3xl font-black text-red-400">{total - correctas}</div>
              <div className="text-xs text-slate-500 mt-1">Incorrectas</div>
            </div>
          </div>

          <div className="flex flex-col gap-3 w-full max-w-xs">
            <button onClick={() => setVistaMovil("revision")}
              className="w-full bg-slate-700 hover:bg-slate-600 text-white font-bold py-3.5 rounded-2xl transition-all flex items-center justify-center gap-2">
              <svg width="16" height="16" fill="none" viewBox="0 0 24 24">
      ...
      {/* Score panel */}
      <div className="hidden md:flex md:w-96 flex-shrink-0 border-r border-slate-800 flex-col items-center justify-center p-10">
        <motion.div initial={{ scale: 0 }} animate={{ scale: 1 }} transition={{ type: "spring", stiffness: 180, delay: 0.1 }} className="text-7xl mb-6">
          {aprobado ? "🎉" : "📚"}
        </motion.div>
        <motion.h2 initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }}
          className={`text-4xl font-black mb-2 ${aprobado ? "text-emerald-400" : "text-red-400"}`}>
          {aprobado ? "¡Aprobaste!" : "No aprobaste"}
        </motion.h2>
        <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.3 }}
          className="text-slate-500 text-center text-sm mb-8 leading-relaxed">
          {aprobado ? "Excelente. Estás listo para el examen real." : "Sigue practicando, ya casi lo logras."}
        </motion.p>

        <motion.div initial={{ scale: 0.8, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} transition={{ delay: 0.3 }}
          className={`w-40 h-40 rounded-full border-4 flex flex-col items-center justify-center mb-8 ${
            aprobado ? "border-emerald-500 shadow-emerald-500/20" : "border-red-500 shadow-red-500/20"
          } shadow-2xl`}>
          <span className="text-5xl font-black text-white">{pct}%</span>
          <span className="text-sm text-slate-500 mt-1">{correctas} / {total}</span>
        </motion.div>
      ...
      {/* Revisión desktop */}
      <div className="hidden md:flex flex-1 overflow-y-auto justify-center px-16 py-10">
        <RevisionPanel preguntas={preguntas} respuestas={respuestas} />
      </div>
    </motion.div>
  );
